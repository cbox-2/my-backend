...
function hovmenu (o, mnu, ishov) {
  ...
  var links = {
    "layout": "/public/admin/layout/",
    "style": "/public/admin/style/",
    "settings": "/public/admin/settings/",
    "dateopt": "/public/admin/dateopt/",
    "smilies": "/public/admin/smilies/",
    "filtering": "/public/admin/filtering/",
    "users": "/public/admin/users/",
    "bans": "/public/admin/bans/",
    "userint": "/public/admin/userint/",
    "posts": "/public/admin/posts/",
    "postsarc": "/public/admin/postsarc/",
    "sticky": "/public/admin/sticky/",
    "channels": "/public/admin/channels/",
    "webhook": "/public/admin/webhook/",
    "snippet": "/public/admin/snippet/",
    "account": "/public/admin/acct/"
  };
  
  var str = '';
  for (var i = 0; i < mnucont[mnu].length / 3; i++) {
    var linkKey = mnucont[mnu][i*3];
    var href = links[linkKey] || "#";
    str += '<a href="' + href + '"><span class="submenuimg" style="background-position: '+mnucont[mnu][i*3+1]+'"></span> '+mnucont[mnu][i*3+2]+'</a>';
  }
  hm.innerHTML = str;
...
function rcvdformresponse(i) {
  ...
  var loc = frames[i] ? frames[i].location.href : '';
  if (loc == "javascript:false" || loc == "about:blank") return false; // first load.
  else if (loc == "javascript:true") setmsg(r, "Timeout. Please try again.", 2);
  else {
    if (frames[i] && frames[i].ld != 1) setmsg(r, "Error. Please try again.", 2);
    if (frames[i] && !frames[i].substaydisabled) {
...
function subsavingfail(i) {
  if (frames["t_"+i]) frames["t_"+i].location.href = "javascript:true;";
}
...
// -------- Have-Session stuff --------
  ...
  var po_box = document.getElementById("popovr_box");
  var po_text = document.getElementById("popovr_text");
  var po_title = document.getElementById("popovr_title");
  

  function popovr(t, x) {
    if (po_box) po_box.style.display = "block";
    if (po_text) po_text.innerHTML = x;
    if (po_title) po_title.innerHTML = t;
  }
  
  function popovr_close() {
    if (po_box) po_box.style.display = "none";
    var el = document.getElementById("popovr_t_login");
    if (el) el.style.display = "none";	// rough hack
  }
  
  function logout() {
    popovr('Log out', '<p>Log out of the control panel?</p><fieldset><div style="float: right;"><input type="button" value=" Yes " onclick="window.location.href=\'/api/logout.php\'"> <input type="button" value=" No " onclick="popovr_close()"></div></fieldset>');
    return false;
  }
...
if (document.forms['qlogin'] && location.protocol == "http:") {
	...
	if (fq['pword'] && fq['pword'].parentElement) {
...
var showSiteError = function (type, text) {
	...
	if (!type && !text) {
		if ($nb) $nb.className = "Hidden";
		return;
	}
	
	var cN = type == 'error' ? 'Error' : '';
	
	if ($nbCont) $nbCont.innerHTML = text + '&nbsp;' + (type == 'error' ? '' : '');
	
	if ($nb) $nb.className = "Visible " + cN;
