...
mnucont[1] = new Array("layout", "-154px -0px", "Layout options", "style", "-0px -0px", "Theme editor");
mnucont[2] = new Array("settings", "-0px -154px", "Posting options", "dateopt", "-176px -0px", "Date options", "smilies", "-0px -132px", "Emoticons", "filtering", "-220px -0px", "Filtering");
mnucont[3] = new Array("users", "-44px -0px", "Registered users", "bans", "-88px -0px", "Blocked users", "userint", "-0px -88px", "User integration");
mnucont[4] = new Array("posts", "-198px -0px", "Messages", "postsarc", "-0px -44px", "Archives", "sticky", "-0px -22px", "Sticky message", "channels", "-22px -22px", "Channels", "webhook", "-0px -178px", "Webhook");
...
function hovmenu(o, mnu, ishov) {
	...
	for (var i = 0; i < mnucont[mnu].length / 3; i++) {
		str += '<a href="/public/admin/'+mnucont[mnu][i+i*2]+'/"><span class="submenuimg" style="background-position: '+mnucont[mnu][i*3+1]+'"></span> '+mnucont[mnu][i*3+2]+'</a>';
	}
	hm.innerHTML = str;
	curmnu = o;
...
function togglemenu() {
	...
	var hm = document.getElementById("hovmenu");
	if (!menuon) { hm.style.display = "block"; menuon = 1; }
	else { menuon = 0; }
}
...
function logout() {
	popovr('Log out', '<p>Log out of the control panel?</p><fieldset><div style="float: right;"><input type="button" value=" Yes " onclick="location.href=\'/public/admin/logout/\'"> <input type="button" value=" No " onclick="popovr_close()"></div></fieldset>');
	return false;
}
...
var upgradeCheckboxes = function() {
	...
	for (var i = 0; i < cbs.length; i++) {
		...
		if (cbs[i].className === "NoReplace") { continue; }
