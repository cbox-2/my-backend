...
function hovmenu (o, mnu, ishov) {
  ...
  for (var i = 0; i < mnucont[mnu].length / 3; i++) {
    str += '<a href="#"><span class="submenuimg" style="background-position: '+mnucont[mnu][i*3+1]+'"></span> '+mnucont[mnu][i*3+2]+'</a>';
  }
  hm.innerHTML = str;
...
// -------- Have-Session stuff --------
  ...
  function logout() {
    popovr('Log out', '<p>Log out of the control panel?</p><fieldset><div style="float: right;"><input type="button" value=" Yes " onclick="window.location.href=\'/\'"> <input type="button" value=" No " onclick="popovr_close()"></div></fieldset>');
    return false;
  }
